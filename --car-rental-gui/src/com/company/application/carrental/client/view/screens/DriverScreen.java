...
public class DriverScreen extends Composite {
	...
	private void buildUI() {
		...
		final ADKPanel driverBgCheckFormPanel = buildBackgroundCheckTab();
		VerticalLayoutContainer advancedContainer = new VerticalLayoutContainer();
		advancedContainer.setScrollMode(ScrollMode.AUTO);
		advancedContainer.add(driverBgCheckFormPanel);

		ADKTabPanel driverTabPanel = new ADKTabPanel("driverTabPanel");

		ADKTabItem driverAppTabItem = new ADKTabItem("driverAppTabItem", "Application");
		driverAppTabItem.setWidth(800);
		driverAppTabItem.setHeight(600);
		driverAppTabItem.add(basicFormPanel);
		driverTabPanel.add(driverAppTabItem);

		ADKTabItem driverBgCheckTabItem = new ADKTabItem("driverBgCheckTabItem", "Background Check");
		driverBgCheckTabItem.setWidth(800);
		driverBgCheckTabItem.setHeight(800);
		driverBgCheckTabItem.add(driverBgCheckFormPanel);
	...
	private ADKPanel buildBasicTab() {
		...
		((ColumnConfig) adkCustomHelpList.getColumnsConfigList().get(0)).setHidden(true);

		EventType specialEvent = new EventType(800);
		adkCustomHelpList.setSpecialEventOnGridSelection(true, specialEvent);
		adkCustomHelpList.getHelpList().addListener(specialEvent, new Listener<BaseEvent>() {

			public void handleEvent(BaseEvent be) {
				mediator.searchDriverApplication((Integer) adkCustomHelpList.getModelValue().get("Driver_Id"));
			}
		});
	...
	public void populateApplicationFormData() {
		...
		primaryContactTextField.setValue(globalDriverClientDto.getPrimaryContactNumber());
		// masterClientDto.setCurrentAreaId(currentAreaTextField.getCurrentValue());
		// masterClientDto.setGender(genderTextField.getCurrentValue());
		...
		licenseExpiryDateField.setValue(globalDriverClientDto.getLicenceExpiryDate());
		medicalHistoryTextArea.setValue(globalDriverClientDto.getMedicalHistory());
		eyeSightTextArea.setValue(globalDriverClientDto.getEyeSight());
		diabetesTextArea.setValue(globalDriverClientDto.getDiabetes());
		accidentHistoryTextArea.setValue(globalDriverClientDto.getAccidentHistory());
